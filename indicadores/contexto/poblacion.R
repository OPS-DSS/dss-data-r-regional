...
process_poblacion <- function(output_dir = here("outputs")) {
  ...
  csv_file <- file.path(output_dir, "csv", "poblacion.csv")
  parquet_file <- file.path(output_dir, "parquet", "poblacion.parquet")

  write_csv(indicador, csv_file)
  write_parquet(indicador, parquet_file)

  message(glue("Context indicator processed: Población total"))
